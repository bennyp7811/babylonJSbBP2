...
function createBoxWithMesh(scene: Scene) {
  const box = MeshBuilder.CreateBox("boxWithMesh", { size: 1 }, scene);
  const material = new StandardMaterial("boxMaterial", scene);
  material.diffuseTexture = new Texture("assets/floor.png", scene);
...
export default function createStartScene(engine: Engine) {
  ...
  let boxWithMesh = createBoxWithMesh(scene);
